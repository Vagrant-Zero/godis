...
type AeFileEvent struct {
	fd    int
	mask  FeType
	proc  FileProc // callback function
	extra interface{}
}

// AeTimeEvent Single LinkedList Node
type AeTimeEvent struct {
	id       int
	mask     TeType
	when     int64    // ms
	interval int64    // ms
	proc     TimeProc // callback function
	extra    interface{}
	next     *AeTimeEvent
}
...
func (loop *AeLoop) AddFileEvent(fd int, mask FeType, proc FileProc, extra interface{}) {
	...
	if err != nil {
		log.Printf("epoll control err: %v\n", err)
		return
	}
	// ae ctl
	var fe AeFileEvent
...
func (loop *AeLoop) RemoveFileEvent(fd int, mask FeType) {
	...
	ev := loop.getEpollMask(fd)
	ev &= ^fe2ep[mask] // 通过bit，扣除已经关注的事件（先取反，再&=）
	if ev != 0 {
		op = unix.EPOLL_CTL_MOD
	}
	err := unix.EpollCtl(loop.fileEventFd, op, fd, &unix.EpollEvent{Fd: int32(fd), Events: ev})
	if err != nil {
		log.Printf("epoll delete err: %v\n", err)
	}
	// ae ctl
